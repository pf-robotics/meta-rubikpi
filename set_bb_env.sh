...
MACHLAYERS=$(find layers -print | grep "meta-rubikpi/conf/machine/.*\.conf" | sed -e 's/\.conf//g' -e 's/layers\///' | awk -F'/conf/machine/' '{print $NF "(" $1 ")"}' | LANG=C sort)
...
DISTROLAYERS=$(find layers -print | grep "meta-rubikpi/conf/distro/.*\.conf" | sed -e 's/\.conf//g' -e 's/layers\///' | awk -F'/conf/distro/' '{print $NF "(" $1 ")"}' | LANG=C sort)
...
#--------------------------------------------------------------
EOF
if [ -e ${WS}/layers/meta-rubikpi/conf/bblayers.conf ]; then
    cat ${WS}/layers/meta-rubikpi/conf/bblayers.conf >> ${BUILDDIR}/conf/bblayers.conf
fi
