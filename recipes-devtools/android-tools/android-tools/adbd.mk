# Makefile for adbd
#
# Forked from meta-openembedded
# (meta-oe/recipes-devtools/android-tools/android-tools/adbd.mk, commit
# 6c9f1f8d45) to build and link libmincrypt. Re-sync with upstream when the
# meta-openembedded submodule is bumped.
...
# libmincrypt provides the AOSP RSA_verify()/SHA implementation used by
# adb_auth_client.c. Without it the 5-argument mincrypt RSA_verify() call is
# silently resolved against OpenSSL's unrelated 6-argument RSA_verify(), which
# crashes adbd as soon as a client presents a key.
VPATH += $(SRCDIR)/system/core/libmincrypt
libmincrypt_SRC_FILES += rsa.c
libmincrypt_SRC_FILES += sha.c
libmincrypt_SRC_FILES += sha256.c
...
# libmincrypt.a must precede -lcrypto so that RSA_verify() binds to the
# mincrypt implementation instead of the OpenSSL symbol of the same name.
LIBS += liblog.a libfs_mgr.a libcutils.a libselinux.a libext4_utils.a libmincrypt.a -lpthread -lbsd -lpcre -lresolv -lcrypto

all: adbd

adbd: liblog.a libfs_mgr.a libcutils.a libselinux.a libext4_utils.a libmincrypt.a $(adbd_OBJS)
